(function(){
  ...
  const STYLE_ID='ab-commandes-document-upload-modal-v47-style';
  let currentOrderId='';
  let busy=false;
  let pendingId='';
  let pendingKey='';
  ...
  function uid47(){try{return typeof uid==='function'?uid():crypto.randomUUID();}catch(_){return Date.now()+'-'+Math.random().toString(36).slice(2);}}
  ...
  function ensureModal(){
    ...
    modal.querySelector('#abUploadSendV47').onclick=sendUpload;
    modal.addEventListener('click',e=>{if(e.target===modal&&!busy)closeUpload()});
    return modal;
  }
  ...
  function selectFile(file){
    const meta=document.getElementById('abUploadFileMetaV47'),send=document.getElementById('abUploadSendV47');
    pendingId='';pendingKey='';status('');
    ...
    if(send){send.disabled=false;send.textContent='Téléverser'}
  }
  ...
  function openUpload(orderId){
    ...
    currentOrderId=String(order.id||'');busy=false;pendingId='';pendingKey='';
  ...
  function closeUpload(){if(busy)return;document.getElementById('abUploadModalV47')?.classList.remove('show');currentOrderId='';pendingId='';pendingKey='';}

  function targetedJsonp(id){return new Promise((resolve,reject)=>{
    const cb='__abUploadV47_'+Date.now()+'_'+Math.random().toString(36).slice(2),s=document.createElement('script');let done=false;
  ...
  async function waitDoc(id){for(let i=0;i<5;i++){if(i)await new Promise(r=>setTimeout(r,550));const d=await findDoc(id);if(d)return d}return null}

  async function sendUpload(){
    if(busy)return;
    const order=orderById(currentOrderId),input=document.getElementById('abUploadFileV47'),file=input?.files?.[0]||null,send=document.getElementById('abUploadSendV47');
    ...
    const key=[order.id,file.name,file.size,file.lastModified].join('|');if(!pendingId||pendingKey!==key){pendingId=uid47();pendingKey=key;}
    busy=true;if(send){send.disabled=true;send.textContent='Envoi…'}
    try{
      status('Vérification…');
      let found=await findDoc(pendingId);
      if(!found){
        status('Envoi vers Google Drive…');
        const base64=await fileToBase64(file);
        const payload={action:'document_upload',id:pendingId,commande_id:String(order.id||''),chantier:String(order.chantier||''),type:fileType(file),file_name:String(file.name||'fichier'),nom_fichier:String(file.name||'fichier'),mime_type:String(file.type||'application/octet-stream'),file_base64:base64,source:'Google Drive'};
        await fetch(GAS,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify(payload)});
        status('Confirmation de l’enregistrement…');found=await waitDoc(pendingId);
      }
      ...
      status('Document enregistré.','ok');
      try{if(typeof renderAll==='function')renderAll()}catch(_){}
      setTimeout(()=>{busy=false;closeUpload()},450);
    }catch(err){
      console.error('AB COMMANDES V47 upload',err);status(err?.message||'Échec de l’envoi.','err');busy=false;if(send){send.disabled=false;send.textContent='Réessayer'}
    }
  }

  injectStyle();ensureModal();ensureLaunchButton();
  try{
    if(typeof openModal==='function'&&!openModal.__abDocUploadV47){const prev=openModal;openModal=async function(){const out=await prev.apply(this,arguments);ensureLaunchButton();requestAnimationFrame(ensureLaunchButton);setTimeout(ensureLaunchButton,80);return out};openModal.__abDocUploadV47=true;}
  }catch(e){console.error('AB COMMANDES V47 open modal',e)}
  try{
    if(typeof closeModal==='function'&&!closeModal.__abDocUploadV47){const prev=closeModal;closeModal=function(){document.getElementById('abUploadModalV47')?.classList.remove('show');currentOrderId='';return prev.apply(this,arguments)};closeModal.__abDocUploadV47=true;}
  }catch(e){}
