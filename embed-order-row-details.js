(function(){
  ...
  function injectStyle(){
    ...
    style.textContent=`
      ...
      body.ab-embed-mode #chantierFiche .ab-row-simple .status-select{
        min-width:0!important;
        width:100%!important;
      }

      body.ab-embed-mode #${MODAL_ID}{
        align-items:flex-start!important;
        justify-content:center!important;
        padding:18px!important;
        overflow:visible!important;
      }
      body.ab-embed-mode #${MODAL_ID} .dialog{
        width:min(430px,calc(100% - 6px))!important;
      ...
      @media(max-width:620px){
        ...
        body.ab-embed-mode #${MODAL_ID}{padding:9px!important}
        body.ab-embed-mode #${MODAL_ID} .dialog{width:100%!important;padding:14px!important}
      }
    `;
  ...
  function ensureModal(){
    ...
    modal.innerHTML=`
      <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="abOrderReadTitle">
        <h3 id="abOrderReadTitle">Détail du produit</h3>
        <div class="ab-read-grid">
          <div class="ab-read-field full"><div class="ab-read-label">Libellé</div><div id="abReadProduit" class="ab-read-value">—</div></div>
          <div class="ab-read-field"><div class="ab-read-label">Quantité</div><div id="abReadQte" class="ab-read-value">—</div></div>
          <div class="ab-read-field"><div class="ab-read-label">Unité</div><div id="abReadUnite" class="ab-read-value">—</div></div>
          <div class="ab-read-field full"><div class="ab-read-label">Prix éventuel</div><div id="abReadPrix" class="ab-read-value">—</div></div>
        </div>
        <div class="ab-read-actions">
          <button id="abReadDelete" type="button" class="btn red">Supprimer</button>
          <button id="abReadClose" type="button" class="btn secondary">Fermer</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
    modal.querySelector('#abReadClose').onclick=closeReadModal;
    modal.onclick=ev=>{if(ev.target===modal)closeReadModal()};
    modal.querySelector('#abReadDelete').onclick=async()=>{
      const id=currentOrderId;
      if(!id)return;
      if(!confirm('Supprimer ce produit ?'))return;
      closeReadModal();
      try{if(typeof deleteOrder==='function')await deleteOrder(id)}catch(err){console.error(err)}
    };
    return modal;
  }
  ...
  function splitQty(raw){
    ...
    if(!s)return {qte:'—',unite:'—'};
    const m=s.match(/^([+-]?(?:\d+(?:[.,]\d+)?|[.,]\d+))\s*([^\d].*)?$/);
    if(!m)return {qte:s,unite:'—'};
    return {qte:m[1]||s,unite:String(m[2]||'').trim()||'—'};
  }

  function orderPrice(o){
    const v=o?.prix??o?.prix_unitaire??o?.price??o?.montant??o?.montant_ht??'';
  ...
  function openReadModal(id){
    ...
    const parsed=splitQty(o.qte);
    const modal=ensureModal();
    modal.querySelector('#abReadProduit').textContent=String(o.produit||'—');
    modal.querySelector('#abReadQte').textContent=parsed.qte;
    modal.querySelector('#abReadUnite').textContent=String(o.unite||o.unité||parsed.unite||'—');
    modal.querySelector('#abReadPrix').textContent=orderPrice(o);
  ...
  function transformRow(row){
    if(!row||row.dataset.abSimpleRow==='1')return;
    const id=String(row.dataset.ficheId||'');
    if(!id)return;
    const arr=(typeof orders!=='undefined'&&Array.isArray(orders))?orders:[];
    const o=arr.find(x=>String(x.id||'')===id);
    if(!o)return;
    const n=typeof docCount==='function'?docCount(o.id):0;
    row.dataset.abSimpleRow='1';
    row.classList.add('ab-row-simple');
    row.innerHTML=`
      <div><button type="button" class="ab-order-label" data-ab-read-id="${e(o.id)}">${e(o.produit||'—')}</button></div>
    ...
    status.onchange=async()=>{
      const current=arr.find(x=>String(x.id||'')===id);
      if(!current)return;
      ...
      try{if(typeof saveOrder==='function')await saveOrder({...current,status:status.value})}catch(err){console.error(err)}
    };
  ...
  function transformRows(){
    document.querySelectorAll('#chantierFiche #ficheOrdersList .order-row[data-fiche-id]').forEach(transformRow);
  }
  ...
  window.__AB_COMMANDES_ORDER_ROW_DETAILS_VERSION='1.0';
})();
