(function(){
  ...
  function hydrateLocalState(){
    try{
      ...
      if(cached.savedAt&&Date.now()-Number(cached.savedAt)>LOCAL_MAX_AGE)return false;
  ...
  function queueOrderUpsert(obj){
    coalesce({entity:'order',action:'upsert',targetId:text(obj.id),payload:obj,createdAt:Date.now(),lastSentAt:0,attempts:0});
  }
  function queueOrderDelete(id){
    coalesce({entity:'order',action:'delete',targetId:text(id),createdAt:Date.now(),lastSentAt:0,attempts:0});
  }
  function queueDocUpsert(obj){
    coalesce({entity:'document',action:'upsert',targetId:text(obj.id),payload:obj,createdAt:Date.now(),lastSentAt:0,attempts:0});
  }
  function queueDocDelete(id){
    coalesce({entity:'document',action:'delete',targetId:text(id),createdAt:Date.now(),lastSentAt:0,attempts:0});
  }
  ...
  function applyState(nextOrders,nextDocs,allowRender=true){
    ...
    if(before===after){saveLocalState();return false;}
    orders=nextOrders;
    documents=nextDocs;
    saveLocalState();
    if(allowRender){
  ...
  async function backgroundLoadAll(silent=true){
    ...
    try{
      ...
      if(!silent||changed||queue.length){
        try{setSync(true,queue.length?'Enregistré localement · synchronisation en arrière-plan':'À jour')}catch(e){}
      }
      return changed;
    }catch(e){
      console.warn('AB COMMANDES · synchro arrière-plan',e);
      if(!silent){try{setSync(false,'Données locales conservées · reprise automatique')}catch(err){}}
      return false;
    }finally{
  ...
  function scheduleVerification(){
    ...
    verifyTimer=setTimeout(()=>backgroundLoadAll(true),VERIFY_DELAY_MS);
  }
  ...
  try{
    saveOrder=async function(obj){
      ...
      saveLocalState();
      if(typeof renderAll==='function')renderAll();
      try{setSync(true,'Enregistré localement · synchronisation en arrière-plan')}catch(e){}
    ...
    deleteOrder=async function(id){
      ...
      saveLocalState();
      if(typeof renderAll==='function')renderAll();
      try{setSync(true,'Suppression enregistrée localement · synchronisation en arrière-plan')}catch(e){}
      setTimeout(flushQueue,0);
      return true;
    };
    ...
    saveDocument=async function(doc){
      ...
      try{setSync(true,'Document enregistré localement · synchronisation en arrière-plan')}catch(e){}
      setTimeout(flushQueue,0);
      return true;
    };
  ...
  try{
    if(typeof loadYayaChantiers==='function'){
      ...
      loadYayaChantiers=async function(force=false){
        const now=Date.now();
        if(!force&&now-lastYayaSync<YAYA_SYNC_MS)return true;
        lastYayaSync=now;
        return originalLoadYayaChantiers(force);
      };
    }
  }catch(e){}
  ...
  setInterval(()=>backgroundLoadAll(true),REMOTE_SYNC_MS);
